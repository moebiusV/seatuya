/*
 * seatuya_nif.c -- Erlang NIF wrapper for libseatuya (Gleam binding)
 *
 * Compile:
 ...
 *
 * Set SEATUYA_LIB env var to override the .so path at runtime.
 */
...
/* Convert a malloc'd C string into Erlang list and free it. */
...
/* tuya_build_message(Ref, Bin, Cmd, Payload, Key) -> {ok, Bin, Size} */
...
ERL_NIF_INIT(seatuya_nif, nif_funcs, &on_load, NULL, NULL, NULL)
